...
/**
 * Is reposnible for handling everything that can happen in level 2
...
public class GameView2 extends SurfaceView implements Runnable {
    ...
    private GameActivity2 gameActivity; //the game activity
    /* Sounds */
    ...
    /* Player */
    private OrangeRocket rocket; // Rocket (player) object

    /* Missiles */
    private List<Missile> missileList; // Will contain all missiles fired

    /* Asteroids */
    private Ufo[] ufoList; //List of all the asteroids present in map


    public GameView2(GameActivity2 gameActivity, int screenX, int screenY) {
        super(gameActivity);
        ...
        ufoList = new Ufo[4]; //initialization of the list
        ...
        bg1 = new Background(screenX, screenY, getResources(), 2);
        bg2 = new Background(screenX, screenY, getResources(), 2);

        rocket = new OrangeRocket(this, screenY, getResources()); // Instantiation of rocket
        ...
        for (int i = 0; i < 4; i++) {
            Ufo a = new Ufo(getResources()); //create new asteroid
            ufoList[i] = a; //add asteroid to list
        }
        ...
        for (Ufo a : ufoList) {
            ...
            if (a.x + a.width < 0) {
                ...
                if (score < 30) {
                    ...
                    while (a.speed < 5 * scRatioX || a.speed > 7 * scRatioX);
                }
                if (score > 30 && score < 100) {
                    do {
                        a.speed = rnd.nextInt(speedLimit); // Random speed inside bounds
                    }
                    while (a.speed < 7 * scRatioX || a.speed > 9 * scRatioX);
                }
                ...
                if (score > 100) {
                    ...
                    while (a.speed < 20 * scRatioX || a.speed > 25 * scRatioX);
                }
    ...
    private void draw() {
        ...
        if (getHolder().getSurface().isValid()) {
            ...
            gameCanvas.drawText("Previous Best: " +
                    sharedPreferences.getInt("highscorelvl2",
                            0), 50, 90, paint);
    ...
    private boolean isHighScore() {
        if (sharedPreferences.getInt("highscorelvl2", 0) < score) {

            SharedPreferences.Editor editor = sharedPreferences.edit(); //edit the preferences
            editor.putInt("highscorelvl2", score); // put the new high score
